...
jQuery( function( $ ) {
  'use strict';
	...
	// WPB copy all SK
	if ( ! $( '.xtra_copy_all_sk' ).length && ( window.location.href.indexOf( 'xtratheme.' ) >= 0 || window.location.href.indexOf( 'themetor.' ) >= 0 || window.location.href.indexOf( 'codevz.' ) >= 0 || window.location.href.indexOf( 'weebtheme.' ) >= 0 ) ) {
						...
						if ( value.indexOf( 'url:' ) >= 0 ) {
							...
							if ( url[ 0 ] ) {
								...
								sk[ name ] = {
									...
									'url': url.replace( 'xtratheme.com/', 'xtratheme.com/elementor/' ).replace( 'themetor.com/', 'themetor.com/elementor/' ).replace( 'codevz.com/', 'codevz.com/elementor/' ),
									'is_external': '',
									'nofollow': '',
									'custom_attributes': ''
		...
		en.off().on( 'click', function( e ) {
		  ...
		  var en    = $( this ),
			  ...
			  reset_kit = function( r ) {
				...
				// Reset Process
				kit_modal.find( '[name]' ).not( '[name="live_id"]' ).val( '' ).trigger( 'change' ).closest( 'form' ).find( '.codevz-icon-preview, .codevz-font-remove, .codevz-icon-remove' ).trigger( 'click' ).find( 'i:not(.fa-remove)' ).removeClass();
