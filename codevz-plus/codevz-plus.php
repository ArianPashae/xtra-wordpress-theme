...
update_option( 'codevz_theme_activation', [
	'purchase_code' => 'xxxxx-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx',
...
class Codevz_Plus {
	...
	protected function __construct() {
		...
		self::$title 		= esc_html__( 'Codevz Plus', 'codevz' );
	...
	public function wp_footer() {
		...
		// Back to top
		echo self::option( 'backtotop' ) ? '<i class="' . esc_attr( self::option( 'backtotop' ) ) . ' backtotop">' . ( is_customize_preview() ? '<i class="codevz-section-focus fas fa-cog" data-section="footer_more"></i>' : '' ) . '</i>' : '';
		...
		if ( $cf7 ) {
			...
			$icon = '<i class="' . esc_attr( self::option( 'cf7_beside_backtotop_icon', 'fa fa-envelope-o' ) ) . ' fixed_contact">' . ( is_customize_preview() ? '<i class="codevz-section-focus fas fa-cog" data-section="footer_more"></i>' : '' ) . '</i>';
	...
	public static function get_page_as_element( $id = '', $query = 0 ) {
		...
		if ( get_post_meta( $id, '_elementor_edit_mode', true ) && did_action( 'elementor/loaded' ) ) {
			...
			return \Elementor\Plugin::instance()->frontend->get_builder_content( $id, false );
	...
	public static function wp_enqueue_scripts() {
		...
		wp_localize_script( $name, 'xtra_strings', array(
			'wishlist_url' 		=> $link,
	...
	public static function tiny_mce_before_init( $i ) {
		...
		$websafe_fonts = apply_filters( 'codevz/field/fonts/websafe', [] );
	...
	public static function _out( $a, $c = '', $s = '', $enqueue = '', $enqueue_extra = '' ) {
		...
		// Check pro version.
		if ( self::is_free() && self::$vc_editable ) {
	...
	public static function ajax_search() {
		...
		$l = empty( $_GET['search_count'] ) ? self::option( 'search_count', 4 ) : esc_attr( $_GET['search_count'] );
	...
	public static function load_font( $f = '' ) {
		...
		$defaults = apply_filters( 'codevz/field/fonts/websafe', array(
			'inherit' 			=> 'inherit', 
			'initial' 			=> 'initial', 
		...
		if ( $font && ! $disable && ! isset( $defaults[ $font ] ) ) {
	...
	public static function pro_badge( $link = true ) {
		...
		if ( $link ) {
			...
			return '<a href="' . esc_url( get_admin_url() ) . 'admin.php?page=theme-activation" target="_blank" class="xtra-pro"><span">' . esc_html__( 'PRO', 'xtra' ) . '</span></a>';
